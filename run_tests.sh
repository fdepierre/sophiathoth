...
run_document_processor_tests() {
    ...
    docker exec sophiathoth_document-processor_1 bash -c "cd /app && export PYTHONPATH=/app && pip install pandas requests openpyxl && python -m unittest discover -s tests/functional"
